...
def build_es_frame_summary(per_frame, es_per_subject, curve_type='risk_coverage'):
    ...
    if curve_type == 'risk_coverage':
        if es_per_subject and 'unc_per_subject' in es_per_subject:
            ...
            # Normalize each subject's curve by its own mean before computing
            # std, then scale back. This captures shape variability across
            # subjects rather than magnitude variability (subjects have very
            # different absolute error levels).
            subj_means = unc_subj.mean(axis=1, keepdims=True)  # [N_subj, 1]
            subj_means = np.clip(subj_means, 1e-12, None)
            normed = unc_subj / subj_means          # normalized to ~1.0
            std_normed = normed.std(axis=0)          # relative std
            std_unc = std_normed * avg_unc           # scale back to data units
        else:
            # Fallback: average over frames, no std
            x = per_frame[frames[0]]['coverages']
        ...
        if es_per_subject and 'unc_per_subject' in es_per_subject:
            ...
            # Normalized std (see risk_coverage branch for rationale)
...
def plot_comparison(backbones, output_dir):
    ...
    for col, bb in enumerate(backbones):
        ...
        if rc is not None:
            ...
            # Layer 1: light σ-band
            ax.fill_between(x, rc_lo, rc_hi,
                            color=C_BAND, alpha=0.30, linewidth=0,
                            label='Uncertainty ($\\pm 1\\sigma$)')
            # Layer 2: hatched nAURC area (subtler)
            ax.fill_between(x, rc['oracle'], rc['unc_mean'],
                            facecolor='none', edgecolor=C_HATCH, linewidth=0.0,
