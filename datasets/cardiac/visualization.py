...
def visualize_uncertainty_cardiac_frames(sampled_velocities, v_reg_list, Sdef_series, series,
    ...
    for i in range(num_cardiac_frames):
        ...
        if i in sampled_velocities and len(sampled_velocities[i]) > 0:
            ...
            #var_mag = np.sqrt(variance[0]**2 + variance[1]**2)
            #var_mag = np.sqrt(variance[0] + variance[1]).T
            var_mag = np.sqrt(variance[0] + variance[1])
            #mean_mag = np.sqrt(mean_v[0]**2 + mean_v[1]**2).T
    ...
    # var_vmin, var_vmax = 0, np.max(all_var_mag)
    # # Note: scaling factors will use per-frame min/max instead of global

    # Percentile-based clipping for variance colorbar range:
    # Use 2nd-98th percentile to focus color range on where the data actually
    # lives, so small but meaningful differences (e.g. 0.1 vs 0.2) get
    # distinct colors instead of being washed out by outliers.
    all_var_vals = np.concatenate([v.ravel() for v in all_var_mag])
    var_vmin = np.percentile(all_var_vals, 2)  #2
    var_vmax = np.percentile(all_var_vals, 90)  #80
    ...
    # var_vmax = max(var_vmax, 0.1)  # Ensure minimum for visualization
    ...
    for i in range(num_cardiac_frames):
        ...
        im_var = axes[6, i].imshow(var_mag, cmap='jet', vmin=0.2, vmax=var_vmax)  #var_vmin
        axes[6, i].axis('off')
        # axes[6, i].imshow(source_img, cmap='gray')
        # #im3 = axes[6, i].imshow(var_mag_norm, cmap='jet', vmin=0, vmax=1, alpha=0.8)  # jet_r
        # im3 = axes[6, i].imshow(var_mag_norm, cmap='jet', vmin=0.0, vmax=1, alpha=1.0)
    ...
    # Colorbar for variance row
    if im_var is not None:
        cbar_var = fig.colorbar(im_var, cax=axes[6, -1], orientation='vertical')
        cbar_var.set_label('Var', fontsize=10)
    else:
        axes[6, -1].axis('off')
    axes[7, -1].axis('off')
    axes[8, -1].axis('off')
    ...
    axes[6, 0].set_ylabel('Variance', fontsize=12)
    ...
    if save_individual:
        ...
        ax_s.axis('off')
        fig_s.savefig(os.path.join(indiv_dir, "source.png"), dpi=150, bbox_inches='tight', pad_inches=0.05)
        plt.close(fig_s)
        ...
        for i in range(num_cardiac_frames):
            ...
            ax_t.axis('off')
            fig_t.savefig(os.path.join(indiv_dir, f"target_frame_{i}.png"), dpi=150, bbox_inches='tight', pad_inches=0.05)
            plt.close(fig_t)
            ...
            ax_d.axis('off')
            fig_d.savefig(os.path.join(indiv_dir, f"deformed_reg_frame_{i}.png"), dpi=150, bbox_inches='tight', pad_inches=0.05)
            plt.close(fig_d)
            ...
            ax_w.axis('off')
            fig_w.savefig(os.path.join(indiv_dir, f"deformed_uq_frame_{i}.png"), dpi=150, bbox_inches='tight', pad_inches=0.05)
            plt.close(fig_w)

            # Uncertainty (variance) map — must match grid plot Row 6 exactly
            fig_u, ax_u = plt.subplots(figsize=(4, 4))
            ax_u.imshow(var_mag, cmap='jet', vmin=0.2, vmax=var_vmax)  #var_vmin
            ax_u.axis('off')
            fig_u.savefig(os.path.join(indiv_dir, f"uncertainty_frame_{i}.png"), dpi=150, bbox_inches='tight', pad_inches=0)
            plt.close(fig_u)

            # Registered velocity magnitude
            v_reg_mag = all_v_reg_mag[i]
            fig_rv, ax_rv = plt.subplots(figsize=(4, 4))
            ax_rv.imshow(v_reg_mag, cmap='jet', vmin=vel_vmin, vmax=vel_vmax)
            ax_rv.axis('off')
            fig_rv.savefig(os.path.join(indiv_dir, f"reg_velocity_frame_{i}.png"), dpi=150, bbox_inches='tight', pad_inches=0.05)
            plt.close(fig_rv)

            # Mean sampled velocity magnitude
            mean_mag = all_mean_mag[i]
            fig_mv, ax_mv = plt.subplots(figsize=(4, 4))
            ax_mv.imshow(mean_mag, cmap='jet', vmin=vel_vmin, vmax=vel_vmax)
            ax_mv.axis('off')
            fig_mv.savefig(os.path.join(indiv_dir, f"mean_velocity_frame_{i}.png"), dpi=150, bbox_inches='tight', pad_inches=0.05)
            plt.close(fig_mv)

            # Grid overlay (UQ mean velocity)
            with torch.no_grad():
                mean_v_tensor_grid = all_mean_v[i]
                disp_list_grid = vec_int(mean_v_tensor_grid)
                displacement_grid = disp_list_grid[-1]
                disp_x_grid = displacement_grid[0, 0]
                disp_y_grid = displacement_grid[0, 1]
                grid_spacing = 4
                y_coords_grid = torch.arange(0, img_size)
                x_coords_grid = torch.arange(0, img_size)
                grid_y_g, grid_x_g = torch.meshgrid(y_coords_grid, x_coords_grid, indexing='ij')
                deformed_x_g = grid_x_g.cuda() + disp_x_grid
                deformed_y_g = grid_y_g.cuda() + disp_y_grid

            fig_gu, ax_gu = plt.subplots(figsize=(4, 4))
            ax_gu.imshow(source_img, cmap='gray', vmin=0, vmax=1)
            for j in range(0, img_size, grid_spacing):
                ax_gu.plot(deformed_x_g[j, :].detach().cpu().numpy(),
                           deformed_y_g[j, :].detach().cpu().numpy(),
                           'r-', linewidth=0.8, alpha=0.9)
            for j in range(0, img_size, grid_spacing):
                ax_gu.plot(deformed_x_g[:, j].detach().cpu().numpy(),
                           deformed_y_g[:, j].detach().cpu().numpy(),
                           'r-', linewidth=0.8, alpha=0.9)
            ax_gu.set_xlim(0, img_size)
            ax_gu.set_ylim(img_size, 0)
            ax_gu.axis('off')
            fig_gu.savefig(os.path.join(indiv_dir, f"grid_uq_frame_{i}.png"), dpi=150, bbox_inches='tight', pad_inches=0.05)
            plt.close(fig_gu)

            # Grid overlay (Registration velocity)
            with torch.no_grad():
                v_reg_tensor_grid = v_reg_list[i][subject_idx:subject_idx+1]
                disp_list_reg_grid = vec_int(v_reg_tensor_grid)
                displacement_reg_grid = disp_list_reg_grid[-1]
                disp_x_reg_grid = displacement_reg_grid[0, 0]
                disp_y_reg_grid = displacement_reg_grid[0, 1]
                deformed_x_rg = grid_x_g.cuda() + disp_x_reg_grid
                deformed_y_rg = grid_y_g.cuda() + disp_y_reg_grid

            fig_gr, ax_gr = plt.subplots(figsize=(4, 4))
            ax_gr.imshow(source_img, cmap='gray', vmin=0, vmax=1)
            for j in range(0, img_size, grid_spacing):
                ax_gr.plot(deformed_x_rg[j, :].detach().cpu().numpy(),
                           deformed_y_rg[j, :].detach().cpu().numpy(),
                           'r-', linewidth=0.8, alpha=0.9)
            for j in range(0, img_size, grid_spacing):
                ax_gr.plot(deformed_x_rg[:, j].detach().cpu().numpy(),
                           deformed_y_rg[:, j].detach().cpu().numpy(),
                           'r-', linewidth=0.8, alpha=0.9)
            ax_gr.set_xlim(0, img_size)
            ax_gr.set_ylim(img_size, 0)
            ax_gr.axis('off')
            fig_gr.savefig(os.path.join(indiv_dir, f"grid_reg_frame_{i}.png"), dpi=150, bbox_inches='tight', pad_inches=0.05)
            plt.close(fig_gr)
        ...
        # Save standalone colorbar with actual variance tick labels
        # Use full range (100th percentile) for colorbar so labels reflect true max
        colorbar_vmax = np.percentile(all_var_vals, 100)
        fig_cb, ax_cb = plt.subplots(figsize=(0.3, 4))
        norm = plt.Normalize(vmin=var_vmin, vmax=colorbar_vmax)
        cb = fig_cb.colorbar(plt.cm.ScalarMappable(norm=norm, cmap='jet'),
        ...
        # Save standalone velocity magnitude colorbar
        fig_vcb, ax_vcb = plt.subplots(figsize=(0.3, 4))
        norm_vel = plt.Normalize(vmin=vel_vmin, vmax=vel_vmax)
        fig_vcb.colorbar(plt.cm.ScalarMappable(norm=norm_vel, cmap='jet'),
                         cax=ax_vcb, orientation='vertical')
        fig_vcb.savefig(os.path.join(indiv_dir, "velocity_colorbar.png"), dpi=150, bbox_inches='tight', pad_inches=0)
        plt.close(fig_vcb)

        print(f"Saved {num_cardiac_frames * 8 + 2} individual images to: {indiv_dir}")
