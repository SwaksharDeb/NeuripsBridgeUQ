"""
Brain analogue of uncertainty_sde_combined_acdc/run_both_tests.py.

Loads per-frame averaged sparsification curves from four BridgeUQ backbones
(TLRN, LTMA, TM, VxM) plus aggregates the low-dimensional UQ method's per-pair
shards, then plots a single multi-panel figure overlaying all five methods'
sparsification curves per frame.

Usage:
    cd /sfs/weka/scratch/swd9tc/Uncertanity_quantification/LightingTemplate_2
    python -m uncertainty_brain_sde.run_both_tests
    python -m uncertainty_brain_sde.run_both_tests --frames 1 3 5
    python -m uncertainty_brain_sde.run_both_tests --rerun_tlrn --num_runs 100
"""
...
METHOD_CONFIG = {
    'tlrn': {
        ...
        'npz': os.path.join(
            ...
            "uncertainty_brain_sde_v2/aggregate/avg_sparsification_curves.npz",
        ),
        'rerun_cwd': PROJECT_ROOT,
        'rerun_module': "uncertainty_brain_sde_v2.test_fast",
    },
    'ltma': {
        ...
        'npz': os.path.join(
            PROJECT_ROOT,
            "2026Experiments/BRAIN/outputs/TLMA_TGrad/basic_brain/visualization/"
            "uncertainty_brain_sde_v2/aggregate/avg_sparsification_curves.npz",
        ),
        'rerun_cwd': PROJECT_ROOT,
        'rerun_module': "uncertainty_brain_sde_v2.test_fast",
    ...
    'tm': {
        ...
        'npz': os.path.join(
            PROJECT_ROOT,
            "2026Experiments/BRAIN/outputs/TransMorph/basic_brain/visualization/"
            "uncertainty_brain_sde_v2/aggregate/avg_sparsification_curves.npz",
        ),
        'rerun_cwd': PROJECT_ROOT,
        'rerun_module': "uncertainty_brain_sde_v2.test_fast",
    ...
    'vxm': {
        ...
        'npz': os.path.join(
            PROJECT_ROOT,
            "2026Experiments/BRAIN/outputs/VoxelMorph/basic_brain/visualization/"
            "uncertainty_brain_sde_v2/aggregate/avg_sparsification_curves.npz",
        ),
        'rerun_cwd': PROJECT_ROOT,
        'rerun_module': "uncertainty_brain_sde_v2.test_fast",
    ...
    'lowdim': {
        ...
        'shards_root': "/scratch/swd9tc/Uncertanity_quantification/"
                       "low_dimension_uncertainty_brain/outputs_mni88",
    },
}
...
DEFAULT_OUTPUT_DIR = os.path.join(
    PROJECT_ROOT, "uncertainty_brain_sde_v2", "comparison_results"
)


def parse_args():
    p = argparse.ArgumentParser(
        description="Brain UQ orchestrator: overlay sparsification curves "
                    "from four BridgeUQ backbones + low-dim method."
    )
...
def _plot_sparsification_comparison(method_data, frames, save_dir, plot_order):
    ...
    ref_handles.append(Line2D([0], [0], color='black', linestyle=':',
                              linewidth=1.2, label='Oracle (per method)'))
    ref_labels.append('Oracle (per method)')

    handles = method_handles + ref_handles
    labels = method_labels + ref_labels
    ...
    base = os.path.join(save_dir, "sparsification_comparison")
...
def main():
    ...
    plot_order = [m for m in PLOT_ORDER if args.plot_wang or m != 'lowdim']
    ...
    for mkey in plot_order:
        ...
        if mkey == 'lowdim':
            ...
            method_data[mkey] = _load_bridgeuq(cfg['npz'])
    ...
    _plot_sparsification_comparison(method_data, frames, args.output_dir, plot_order)
